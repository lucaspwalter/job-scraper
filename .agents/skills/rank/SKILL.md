---
name: rank
description: Pontua e ordena vagas coletadas contra perfil e critérios do candidato. Use quando usuário pedir ranking ou shortlist de vagas.
---
...
`os argumentos fornecidos pelo usuário` may contain:
...
Dispatch parallel subagentes via subagentes do Codex, ~5 jobs per agent (a single agent is fine for ≤5 jobs). Token-efficiency rules, consistent with `$apply`:
...
- Agents fetch each posting URL with acesso web and score **only from actually fetched content**. If a URL is dead, redirects to a listing page, or the posting has expired, the agent marks that job `expired` - it never scores from the title alone and never fabricates posting content.
