---
name: interview
description: Prepara entrevista específica usando vaga, candidatura enviada, perfil e histórico real. Use quando usuário pedir preparação ou simulação de entrevista.
---
...
`os argumentos fornecidos pelo usuário` may contain a company name (optionally with a role), e.g. `$interview acme`.
...
**Verify before using:** every company claim that will appear in the prep pack must be independently confirmed via fontes web ao vivo - same rule the repo applies to cover-letter claims. An unverified "fact" delivered confidently in an interview is worse than no fact.
