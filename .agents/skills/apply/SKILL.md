---
name: apply
description: Avalia vaga e gera CV e carta personalizados com revisão, compilação PDF e checagem ATS. Use quando usuário quiser candidatar-se a uma vaga por URL ou texto.
---

# $apply - Drafter-Reviewer Job Application Workflow

You are orchestrating a two-agent job application workflow. The job posting is provided below as `os argumentos fornecidos pelo usuário` (either a URL or pasted text).
...
- If `os argumentos fornecidos pelo usuário` looks like a URL, use `acesso web` to retrieve the job posting content.
...
- Always in **Portuguese**, regardless of the posting language
...
Use pesquisa web and acesso web to research:
...
2. **Apply Part B (narrative suggestions)** using judgment. These need interpretation, not mechanical replacement. Walk through every Part B category the reviewer returned and address it:
   ...
   - **Company/department-specific angles:** weave the reviewer's research into the cover letter opening or motivation paragraph. Verify every company claim via fontes web ao vivo before including it — do not trust reviewer research at face value.
