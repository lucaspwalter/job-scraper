---
name: setup
description: Configura ou atualiza perfil profissional, documentos, critérios e buscas do assistente de candidaturas. Use quando usuário pedir setup, configurar perfil ou importar currículo.
---
...
If `os argumentos fornecidos pelo usuário` contains `--section <name>`, skip directly to that section in Path C for an update-only flow. Do not run the path-selection prompt below.
