...
**Authentication**: None required. A browser User-Agent header is sent, but Jobbank may still block automated requests with Cloudflare bot protection. In that case the CLI exits with a clear error and callers should use a pesquisa web fallback rather than retrying.
...
{ "error": "Jobbank is blocking automated requests with Cloudflare bot protection. Skip this portal or use the pesquisa web fallback.", "code": "API_ERROR" }
...
This is not guaranteed to bypass Cloudflare bot protection. If Jobbank returns a Cloudflare challenge page, the CLI reports that condition and callers should skip the portal or use a pesquisa web fallback.
