---
name: add-portal
description: Investiga e cria skill CLI para portal de vagas local, respeitando acesso e termos. Use quando usuário quiser integrar novo portal.
---
...
`os argumentos fornecidos pelo usuário` may contain a subcommand, a portal URL, or nothing.
...
- If `os argumentos fornecidos pelo usuário` contains `--list`: use Glob with `.agents/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
- If `os argumentos fornecidos pelo usuário` contains a URL: treat it as the portal URL and carry it into Step 1.
...
Ask the user (skip anything already answered by `os argumentos fornecidos pelo usuário`):
...
Do reconnaissance before writing any code. Use acesso web (or `curl` via Bash) on the portal:
...
- **`SKILL.md` frontmatter:** `name`, `version: 1.0.0` e `description` escrita para disparo da skill; deve nomear portal, mercado e frases de ativação em inglês e no idioma local.
