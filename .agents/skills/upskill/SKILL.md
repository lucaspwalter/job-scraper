...
### Targeted mode
1. Use acesso web to retrieve the job posting from the URL.
...
### Aggregate mode
For each job row in the tracker, you do not have the full posting — use the `role`, `sector`, and `notes` columns to infer likely required skills. If the row has a `source` URL, you may optionally acesso web it for more detail, but skip if the URL is missing or dead.
...
1. **Run a pesquisa web** to find current, highly-rated study resources. Use queries like:
...
1. **Never fabricate resources.** Only cite resources found via actual pesquisa web results. Do not invent course names, URLs, or authors.
2. **Search with the current year.** Include the year in every pesquisa web query for resources so results stay fresh.
