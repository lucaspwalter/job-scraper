---
name: add-template
description: Registra, testa e ativa template LaTeX de CV ou carta. Use quando usuário quiser adicionar ou trocar template.
---
...
`os argumentos fornecidos pelo usuário` may contain a subcommand, a file path, or nothing.
...
- If `os argumentos fornecidos pelo usuário` contains `--list`: run **List Mode** below and stop.
- If `os argumentos fornecidos pelo usuário` contains `--use <name>`: run **Switch Mode** below, then continue to **Step 5: Activate** with the resolved template metadata. `--use default` deactivates any custom template and restores the stock guidance (see Step 5).
- If `os argumentos fornecidos pelo usuário` contains a file path or @-mentioned file: treat it as the template source and carry it into Step 1.
...
If `os argumentos fornecidos pelo usuário` contains `--use <name>`:
