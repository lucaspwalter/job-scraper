...
`.agents/skills/` (plus pesquisa web as a fallback), using queries from your profile.
...
**Use the installed CLI tools as the primary search mechanism.** Fall back to `pesquisa web` only for portals that do not have a CLI skill, or if `bun` is unavailable on the system.
...
If this fails (bun not installed), skip to **1c (pesquisa web fallback)** for all portals and note the fallback in the Step 5 output.
...
Run all portal CLI calls in parallel where possible using subagentes do Codex. Collect all `results` arrays into a single pool for Step 2, keeping each result tagged with its source portal skill (for Step 2 `detail` lookups).

If a CLI tool exits with a non-zero code, log the error message and continue — do not abort the whole search.

#### 1c. pesquisa web fallback

Use `pesquisa web` for:
...
Use the site-specific query strings from `search-queries.md` directly as pesquisa web queries for these portals.
...
**From pesquisa web results:** Use `acesso web` on the posting URL and extract the same
fields manually.
...
1. **Never fabricate job postings.** Only present jobs from actual CLI search/detail output or fontes web ao vivo results.
2. **Respect deduplication.** Always check seen_jobs.json AND job_search_tracker.csv before presenting.
3. **Focus on configured geographic area.** Skip jobs that require relocation or are clearly outside commute range.
4. **Only open positions.** Skip postings with expired deadlines or those marked as closed.
5. **Be efficient with detail fetches.** Don't run `detail` or acesso web on every search hit — pre-filter by title/snippet, then fetch only promising matches.
6. **Parallel searches.** Run portal CLI searches in parallel; use pesquisa web only for gaps the CLIs don't cover.
