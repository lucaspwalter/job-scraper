...
def fetch_jobs(search_term: str, url: str | None = None) -> list[dict]:
    with httpx.Client(headers=BROWSER_HEADERS, timeout=20, follow_redirects=True) as client:
        response = client.get(url or build_search_url(search_term))
...
def scrape_jobs(search_term: str, url: str | None = None) -> list[dict]:
    ...
    for job in fetch_jobs(search_term, url):
        title = job.get("title")
        job_url = job.get("url")
